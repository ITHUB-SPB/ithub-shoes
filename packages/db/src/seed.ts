...
async function seedLookups(rows: RawData[]): Promise<LookupMaps> {
    ...
    await Promise.all(brandNames.map((title) => prisma.brand.upsert({ where: { title }, update: {}, create: { title } })))
    await Promise.all(categoryNames.map((title) => prisma.category.upsert({ where: { title }, update: {}, create: { title } })))
    await Promise.all(genderNames.map((title) => prisma.gender.upsert({ where: { title }, update: {}, create: { title } })))
    ...
    return {
        ...
        categories: new Map(categories.map((category) => [category.title, category.id])),
...
async function seedProducts(rows: RawData[], lookups: LookupMaps) {
    ...
    for (const row of rows) {
        ...
        if (!brandName || !model || !categoryName || !genderName || !sizeValue || !colorName) {
            continue
        }
